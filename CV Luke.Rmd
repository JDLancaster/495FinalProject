...
x <- this %>% select(PdDistrict, DayOfWeek, Hour) %>% 
  mutate(PdDistrict=as.factor(PdDistrict),DayOfWeek=as.factor(DayOfWeek))

table(x[,3])

table(this$Category)

fit = glmnet(data.matrix(x),as.factor(this$Category), family = "multinomial", type.multinomial = "grouped")
plot(fit, xvar = "lambda", label = TRUE, type.coef = "2norm")

cvfit=cv.glmnet(data.matrix(x),as.factor(this$Category), family="multinomial", type.multinomial = "grouped", parallel = TRUE)
plot(cvfit)
