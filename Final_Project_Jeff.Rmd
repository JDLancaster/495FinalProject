...
From the hour of the day breakdown we notice that most crimes occur during the day (from 8 am to around 6 pm). This is the time window when people commute to work and are at work.   One thing that we thought was interesting was that crime is lowest at around 10pm, and really started to pick back up in the late night / early morning around 3 and 4am.  Since some crimes will probably be much more likely to occur at certain times, we think that this variable will be a good predictor of crime type.
...
From the above plots, `PdDistrict`, `DayOfWeek` and `Hour` seem to indicate when thefts are highly to occur. Since our broad definition of thefts account for most crimes reported, we believe that these variables would be good indicators of the type of crime likely to have been committed.  Below is a map of the police districts in San Fransico - we can see that most of the crime is concentrated at the top right.  Another interesting tidbit is that the richest district in the city is the Presidio - which is the only part of the map below without a police district assigned to it.

![Map of San Francisco PD Districts](https://hoodwork-production.s3.amazonaws.com/uploads/story/image/10247/sfpdfinal.png)
```{r}
